...
/**
 * ThemeToggle -- compact cycle button for the sidepanel header. Each
 * click advances light -> dark -> system -> light. Shares the popup's
 * useTheme hook, so the change propagates to every open extension
 * surface via the THEME_CHANGED broadcast + chrome.storage.onChanged
 * listeners we wired in commit a125311.
 */
...
function IconFor({ theme }: { theme: 'light' | 'dark' | 'system' }): React.ReactElement {
  ...
  if (theme === 'dark') {
    ...
    return (
      <svg width="14" height="14" viewBox="0 0 14 14" aria-hidden="true">
        <path
          d="M11 9A5 5 0 0 1 5 3 5 5 0 1 0 11 9Z"
          fill="currentColor"
        />
      </svg>
    );
  ...
  return (
    <svg width="14" height="14" viewBox="0 0 14 14" aria-hidden="true">
      <rect x="1.5" y="3" width="11" height="7" rx="1" fill="none" stroke="currentColor" strokeWidth="1.2" />
